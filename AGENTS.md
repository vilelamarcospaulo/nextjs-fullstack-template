...
The Next app is the job **producer**: `src/lib/queue.ts` pushes messages onto the "hello" queue via the `HELLO_QUEUE` producer binding declared in `wrangler.app.jsonc` (`queues.producers`), read through `getCloudflareContext().env.HELLO_QUEUE` — the same pattern `src/lib/db.ts` uses for the `HYPERDRIVE` binding. No account id, API token, or queue id — the binding is the credential. Under plain `next dev`, that binding resolves to a Miniflare-simulated queue with no consumer attached (a separate, non-shared process from the actual consumer's own `wrangler dev`), so `sendJob` deliberately refuses to use it there (detected via `env.NEXTJS_ENV === "development"`) and requires `QUEUE_LOCAL_PUSH_URL` instead (local dev only — see `.env.example`): a plain `fetch` against the Worker's own local-only push simulation, so the whole producer/consumer loop runs offline with no Cloudflare account. `opennextjs-cloudflare preview` and real deployments use the real binding.

The **consumer** is a separate Cloudflare Worker, `src/worker/index.ts`, deployed independently via `wrangler deploy` (config in the root `wrangler.toml`) — it is no longer a Node process run alongside the app.
