...
// independently (pnpm run test:unit / test:integration / test:ui /
  ...
  test: {
    ...
    projects: [
      ...
      },
      {
        ...
        // `pnpm run app:build` (runs opennextjs-cloudflare build). The test
        // requires a fresh build to exist. Re-run `pnpm run app:build` if the
