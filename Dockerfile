...
# =============================================================================
# Stage 5 — worker  (target: worker)
# Long-running background job consumer (pg-boss over the same Postgres
# instance the app uses). Runs via Node 24's native TypeScript stripping —
# exactly like the `migrator` stage's CLI invocation and `prisma/seed.ts` —
# so no build/bundle step is needed. It needs:
#   - node_modules (pg-boss, prisma client runtime, pino, etc. from deps)
#   - the full src/ tree: the worker imports across src/lib and src/internal,
#     conditionally loads src/instrumentation.node.ts for OTEL, and needs
#     src/generated/prisma (already generated by the builder stage)
#   - package.json (read by some tooling for metadata; harmless to include)
# DATABASE_URL is supplied at runtime via environment / Compose, same as the
# migrator and runner stages.
# =============================================================================
...
# Must match (or be overridden to match) whatever WORKER_HEALTH_PORT the
# worker's own health-server.ts defaults to / whatever Compose sets.
...
# Simple Node-based health check — avoids a curl/wget dependency in slim.
# Hits the worker's own /healthz endpoint, mirroring the runner stage's idiom.
HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
    CMD node -e "require('http').get('http://127.0.0.1:3100/healthz', (r) => process.exit(r.statusCode >= 200 && r.statusCode < 400 ? 0 : 1)).on('error', () => process.exit(1))"
