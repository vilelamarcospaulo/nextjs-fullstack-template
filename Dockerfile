...
# exactly like the `migrator` stage's CLI invocation and `drizzle/seed.ts` —
# so no build/bundle step is needed. It needs:
#   - node_modules (pg-boss, pg driver, pino, etc. from deps)
#   - the full src/ tree: the worker imports across src/lib and src/internal,
#     conditionally loads src/instrumentation.node.ts for OTEL
